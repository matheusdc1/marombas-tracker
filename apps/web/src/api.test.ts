import { describe, expect, it, vi } from 'vitest'
import {
  addMeal,
  addSet,
  addWater,
  addWeight,
  deleteMeal,
  deletePhoto,
  deleteSet,
  getExercises,
  getFoods,
  getGoals,
  getMetrics,
  getPhotos,
  getPrs,
  getReport,
  putGoals,
  putWorkoutDuration,
  sendChat,
  todayIso,
  updateMeal,
  updateSet,
  uploadPhoto,
} from './api'
import { EXERCISES, FOODS, GOALS, METRICS, PHOTOS, PRS, REPORT, mockFetch } from './__tests__/helpers'

describe('api', () => {
  it('monta as URLs e métodos certos', async () => {
    const fn = mockFetch({
      'GET /api/foods': FOODS,
      'GET /api/log/2026-07-06': REPORT,
      'GET /api/metrics': METRICS,
      'GET /api/exercises': EXERCISES,
      'GET /api/prs': PRS,
      'GET /api/photos': PHOTOS,
      'POST /api/log/2026-07-06/meals': { id: 1 },
      'POST /api/log/2026-07-06/sets': { id: 2, is_pr: true },
      'POST /api/chat': { reply: 'ok', meals_logged: 0, sets_logged: 0, unmatched: [] },
      'POST /api/weight': { id: 4 },
      'POST /api/photos': PHOTOS[0],
      'DELETE /api/meals/7': { ok: true },
      'DELETE /api/sets/8': { ok: true },
      'DELETE /api/photos/9': { ok: true },
      'PUT /api/meals/7': { ok: true },
      'PUT /api/sets/8': { ok: true },
      'PUT /api/log/2026-07-06/workout': { ok: true },
      'GET /api/goals': GOALS,
      'PUT /api/goals': { ok: true },
      'POST /api/log/2026-07-06/water': { id: 3 },
    })
    expect(await getFoods('fran go')).toEqual(FOODS)
    expect(fn).toHaveBeenLastCalledWith('/api/foods?q=fran%20go', undefined)
    expect(await getReport('2026-07-06')).toEqual(REPORT)
    expect(await getMetrics('exercicio', 30, 'supino reto')).toEqual(METRICS)
    expect(fn).toHaveBeenLastCalledWith(
      '/api/metrics?metric=exercicio&days=30&exercise=supino%20reto',
      undefined,
    )
    expect(await getExercises()).toEqual(EXERCISES)
    expect(await getPrs()).toEqual(PRS)
    expect(await getPhotos()).toEqual(PHOTOS)
    expect(await addWeight('2026-07-06', 82.5)).toEqual({ id: 4 })
    expect(await putWorkoutDuration('2026-07-06', 72)).toEqual({ ok: true })
    expect(await deletePhoto(9)).toEqual({ ok: true })
    const photo = await uploadPhoto('2026-07-06', 'Frente', new File(['x'], 'a.png'))
    expect(photo).toEqual(PHOTOS[0])
    expect((fn.mock.lastCall![1] as RequestInit).body).toBeInstanceOf(FormData)
    expect(await addMeal('2026-07-06', 1, 150, 'Café da manhã')).toEqual({ id: 1 })
    expect(JSON.parse((fn.mock.lastCall![1] as RequestInit).body as string)).toEqual({
      food_id: 1,
      grams: 150,
      meal_type: 'Café da manhã',
    })
    expect(
      await addSet('2026-07-06', {
        exercise: 'supino',
        sets: 2,
        reps: 10,
        weight_kg: 60,
        rest_s: 90,
      }),
    ).toEqual({ id: 2, is_pr: true })
    expect(await sendChat('2026-07-06', 'comi 100g de arroz')).toMatchObject({ reply: 'ok' })
    expect(await deleteMeal(7)).toEqual({ ok: true })
    expect(await deleteSet(8)).toEqual({ ok: true })
    expect(await updateMeal(7, 2, 120, 'Jantar')).toEqual({ ok: true })
    expect((fn.mock.lastCall![1] as RequestInit).method).toBe('PUT')
    expect(
      await updateSet(8, { exercise: 'remada', sets: 3, reps: 8, weight_kg: 40, rest_s: null }),
    ).toEqual({ ok: true })
    expect(await getGoals()).toEqual(GOALS)
    expect(
      await putGoals({ kcal: 3000, protein_g: 180, water_ml: 3500, carbs_g: 310, fat_g: 75 }),
    ).toEqual({ ok: true })
    expect(await addWater('2026-07-06', 250)).toEqual({ id: 3 })
    expect(JSON.parse((fn.mock.lastCall![1] as RequestInit).body as string)).toEqual({ ml: 250 })
  })

  it('prefixa as URLs com VITE_API_URL quando definido', async () => {
    vi.stubEnv('VITE_API_URL', 'https://api.exemplo.app')
    const fn = mockFetch({ 'GET https://api.exemplo.app/api/goals': GOALS })
    expect(await getGoals()).toEqual(GOALS)
    expect(fn).toHaveBeenLastCalledWith('https://api.exemplo.app/api/goals', undefined)
  })

  it('lança erro quando a resposta não é 2xx', async () => {
    mockFetch({})
    await expect(getReport('2026-07-06')).rejects.toThrow('Erro 404')
  })

  it('todayIso devolve a data local em YYYY-MM-DD', () => {
    const d = new Date()
    const expected = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(
      d.getDate(),
    ).padStart(2, '0')}`
    expect(todayIso()).toBe(expected)
  })
})
